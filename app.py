...
# dashboard.html 內建的自動調整高度腳本（直接改 iframe 自身高度）是主要機制，
...
#
# scrolling=True 是保險網：手機瀏覽器上 JS 量測內容高度的時機常常比桌機慢、不穩定，
# 一旦量到的高度比實際內容矮，內容就會被「定死」在那個高度、無法顯示完整內容。
# 開啟 scrolling=True 之後，就算自動調高沒抓準，使用者還是能在 iframe 內部往下滑
# 看到剩下的內容，不會整段被截斷看不到——這是唯一能保證「內容一定看得到」的作法。
